...
/**
 * admin 用の面談パック管理画面 Controller。CRUD と状態遷移(publish / archive / unarchive)を提供する。
 *
 * ルートパラメータ名は画面(`resources/views/meeting-pack/management/`)が前提とする `{plan}` に合わせている。
 */
class MeetingPackController extends Controller
{
    ...
    public function unarchive(MeetingPack $plan, UnarchiveAction $action): RedirectResponse
    {
        ...
        return redirect()
            ->route('admin.meeting-packs.show', $plan)
            ->with('success', '面談パックのアーカイブを解除しました。');
    }
}
